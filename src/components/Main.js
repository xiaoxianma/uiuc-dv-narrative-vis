...
//scatter plot scale
        ...
        var trendData = [{
            "family": {"x": 0.03, "y": -0.16114577322807189},
            "gdp": {"x": 0.03, "y": 0.13771722698313174},
            "freedom": {"x": 0.03, "y": 0.19385022340720828},
            "health": {"x": 0.03, "y": 0.1449102798209366},
            "generosity": {"x": 0.03, "y": 0.4622831394921143},
            "trust": {"x": 0.03, "y": 0.43740109364034913}
        }, {
            "family": {"x": 0.99, "y": 0.79779990255437661},
            "gdp": {"x": 0.99, "y": 0.94724520414798952},
            "freedom": {"x": 0.99, "y": 0.77578394648940319},
            "health": {"x": 0.99, "y": 0.85635149202827043},
            "generosity": {"x": 0.99, "y": 0.78474449144773484},
            "trust": {"x": 0.99, "y": 0.90127344573029378}
        }];
        ...
        function link(scope, element, attrs) {
            ...
            function radarChart(radarData, radarCountryId, options) {
                ...
                var maxValue = Math.max(cfg.maxValue, d3.max(radarData, function (i) {
                    return d3.max(i.map(function (o) {
                        return o.value;
                    }))
                }));

                var allAxis = (radarData[0].map(function (i, j) {
                        return i.axis
                    })), //Names of each axis
                ...
                axis.append("text")
                    ...
                    .attr("dy", "0.3em")
                    .attr("x", function (d, i) {
                        return .001 + rScale(maxValue * cfg.labelFactor) * Math.cos(angleSlice * i - Math.PI / 2);
                    })
                    .attr("y", function (d, i) {
            ...
            function radarButtons() {
                ...
                var radarButtons = radarButtonSvg.append("g")
                    ...
                    .attr("opacity", function (d, i) { //setting inital opactiy when radarChart is activated
                        if ((i == 0) && (predictionMode == true)) {
                            return buttonPressedAlpha;
                        }

                        if ((i == 1) && (showRadarMultiple == true)) {
                            return buttonPressedAlpha;
                        } else {
                            return buttonDefaultAlpha;
                        }
                ...
                radarButtons.append("text")
                    ...
                    .style("opacity", 0)
                    .text(function (d, i) {
                        return radarButtonsText[i];
                    });

                radarButtons.append("svg:image")
                    .attr("x", function (d, i) {
                        return 0 + (bWidth + bSpace) * i;
                    })
                    .attr("y", y0)
                    .attr('width', bWidth)
                    .attr('height', bHeight)
                ...
                function handleMouseOver(d, i) {
                    if (scope.data[d.id]) {
                        ...
                        if (modeStatus == 0) {
                            ...
                            svg.append("text")
                                .attr({
                                    id: "globeTextId",  // Create an id for text so we can select it later for removing on mouseout
                                    class: "globeAxis",
                                    x: function () {
                                        return 5;
                                    },
...
// update infoTextDiv and rightDiv plot after clicking the map or point
    ...
    return (
        <svg viewBox="0 0 1600 900" ref={ref}/>
    );
}